...
const EditAgentModal = ({
  ...
  const [form, setForm] = useState<Partial<AgentsRow>>({ ...agent });
  
  // String states for JSON fields
  const [toolsJsonStr, setToolsJsonStr] = useState(JSON.stringify(agent.tools_json || [], null, 2));
  const [memoryFlagsStr, setMemoryFlagsStr] = useState(JSON.stringify(agent.memory_flags || {}, null, 2));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ submit: 'Failed to update agent' });
    } finally {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={form.swarm_group as string || ''}
                  onChange={(e) => updateForm('swarm_group', e.target.value)}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
                  placeholder="Enter swarm group name"
                />
                <p className="text-xs text-gray-500 mt-1">Group agents together for coordinated operation</p>
              </div>
